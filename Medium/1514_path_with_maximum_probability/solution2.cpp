 ...
 * Date:   27, August, 2024
 * Method: Use maxProb to record the maximum probability of each node, so we can avoid the duplicate nodes in the priority queue.
    ...
    double maxProbability(int n, vector<vector<int>>& edges, vector<double>& succProb, int start_node, int end_node) {
        ...
        while (!pq.empty()) {
            ...
            for (auto& v: graph[u.second]) {
                ...
                if (v.second * u.first > maxProb[v.first]) {
                    maxProb[v.first] = v.second * u.first;
