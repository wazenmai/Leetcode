 ...
 * Method: Bottom-up DP. State: dp(i, w) = the minimum cost to paint `w` walls when considering index `i` and beyond
...
public:
    int paintWalls(vector<int>& cost, vector<int>& time) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            for (int j = n; j >= 0; j--) {
                // cout << i << "," << j << "\n";
                int paint = (j - 1 - time[i] <= 0) ? cost[i] : cost[i] + dp[i + 1][j - 1 - time[i]];
